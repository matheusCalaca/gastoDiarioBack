...
public interface GastoRepository extends JpaRepository<GastoEntity, Long> {
    ...
    @Query("SELECT g FROM GastoEntity g WHERE g.user = :user AND MONTH(g.dataCompra) = :month")
    List<GastoEntity> findByUserAndDataCompraMonth(@Param("user") UserEntity user, @Param("month") int month);
}
